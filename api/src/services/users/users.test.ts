...
describe('users', () => {
  ...
  scenario('creates a user', async (scenario: StandardScenario) => {
    const result = await createUser({
      input: {
        ...
        salt: 'String',
        updatedAt: '2025-01-04T12:49:38.799Z',
        defaultOrganizationId: scenario.user.two.defaultOrganizationId,
      },
    })

    expect(result.email).toEqual('String7209118')
    expect(result.hashedPassword).toEqual('String')
    expect(result.salt).toEqual('String')
    expect(result.updatedAt).toEqual(new Date('2025-01-04T12:49:38.799Z'))
  ...
  scenario('updates a user', async (scenario: StandardScenario) => {
    ...
    const result = await updateUser({
      id: original.id,
      input: { email: 'String91041022' },
    })

    expect(result.email).toEqual('String91041022')
  })
